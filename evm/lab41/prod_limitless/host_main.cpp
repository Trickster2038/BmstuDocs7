...
// ===== < CUSTOM PART > =====
	...
	__foreach_core(group, core){
		...
		int num1, num2;
		std::string line;
		std::ifstream myfile("/iu_home/iu6031/Astakhov/worksp/lab41/btwc-dijkstra-xrt/host/src/kronecker_var01.txt");
		if(myfile.is_open())
		{
			/*
				[?] issue: why segmaentation fails if (messages_count > 1900) [?]
			*/
			while((getline(myfile,line)) && (messages_count<10000))
			{	
				// printf(">>> alloating N_%d \n", messages_count);
				num1 = std::atoi(line.substr(0, line.find("\t")).c_str());
				num2 = std::atoi(line.substr(line.find("\t")+1).c_str());
				//printf("key: %d val: %d\n", num1, num2);
				EDGE(num1, num2, 1); 
				EDGE(num2, num1, 1); 
				messages_count++;
		...
		printf(">>> external_memory_sync_to_device \n");
	...
	__foreach_core(group, core) {
		printf(">>> mq_send(2 * sizeof(int)*messages_count)");
		lnh_inst.gpc[group][core]->mq_send(2 * sizeof(int)*messages_count);
	}
